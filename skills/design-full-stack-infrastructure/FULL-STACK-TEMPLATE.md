...
*Style contract — this report is a decision record, not a novel: one fact per bullet, two lines max; rationale is a single "why: …" clause, never argued prose; anything enumerable goes in a table with short cells; the reconciled seam is drawn (≥ 1 mermaid diagram). A bullet that needs a third line is two facts — split it.*
...
**Recovery & stage:** {per shared row: RPO/RTO ← the stricter plane's stated target · the stage the tenant touches — production instance behind its scoped credential, or a non-prod instance (its own deployment, priced)}.
...
**The seam, drawn** — redraw to the verdicts: one node per *shared* row; vetoed/single-plane categories stay inside their plane's box:

```mermaid
flowchart TB
  subgraph CP["coding plane — its own report"]
    A["agents"]
  end
  subgraph PP["product plane — its own report"]
    P["product runtime"]
  end
  subgraph SS["shared substrate — one deployment per shared verdict"]
    S1["{category: component @ reconciled size}"]
  end
  P ==>|"owner"| S1
  A -->|"tenant — scoped credential"| S1
```
...
- [ ] **Recovery & stage reconciled** — every shared row carries the stricter plane's stated RPO/RTO and names the stage the tenant touches; a non-prod instance is counted as its own deployment, never hidden inside the share.
...
- [ ] **Style contract kept** — no bullet or table cell exceeds two lines; rationale rides as *why:* clauses; the seam is drawn.
