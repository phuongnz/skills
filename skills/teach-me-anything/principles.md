...
2. **The mini challenge** — always the same task: *summarise, in your own words, what this lesson taught and why it matters to you.* Not a question of another shape, **not code, not a config file** — the summary is where understanding shows. Practice of *doing* — configuring, calculating, verifying — is real and needed, but it lives in the lesson's lab task or a drill, beside the summary, never in its place ([SKILL.md → Read the verb](./SKILL.md#lessons)). Summarising works because it cannot be faked: to write it the learner has to walk back through the lesson's points, and they can only summarise what they actually understand — so the block tells them to **ask about anything unclear first**, and a learner who stalls at the summary has just found their own gap. Free recall in the learner's own words is generation, not recognition, and generation is where capability actually forms (the *generation effect*). Keep it to a minute or two, then reveal a model answer to compare against.

If the learner turns the summary off in their [preferences](./SKILL.md#preferences), honour it: the quick checks then carry the lesson alone.

Both sit inside a **feedback loop** — the learner acts and finds out how they did at once, ideally automatically. The summary, and any idea a check caught them out on, are the candidates for the [retention engine](./SKILL.md#the-retention-engine) — offered to the learner, who decides what goes in.
