...
- A deadline sets **pace**: keep milestones-left against weeks-left in `NOTES.md`, and say out loud when the learner is behind. **Coverage is fixed; the date moves.** Pace never trims what gets taught — if the hours do not fit, the honest move is a later date, not a thinner milestone. At the diagnostic, put the total (hours a week × weeks) beside what this exam usually takes, and say plainly if it is below range. For an exam, the workspace deliberately holds no exam-format practice — so book it outside: a weekly slot in the pace, a practice-exam source in `sources.html`, and ask each session how the last run went; the exam date should follow those scores, not the calendar.
