...
- `bin/` — `study`, the one command that starts a session: it serves the workspace locally and puts the tutor terminal on the same page; and `check-quiz`, which measures whether a page's options give the answer away ([assessments](#milestones-and-assessments)). Copied from this skill's `templates/bin/` at setup — see [The Tutor Terminal](#the-tutor-terminal).
...
- `assets/` — a shared `style.css` (one stylesheet for every page), `md.js` (a tiny offline Markdown renderer) and `themes/` (the colour sets the learner picks from). Copied from this skill's `assets/` once at setup; leave them untouched per workspace.
...
- `preferences.html` — the learner's **settings**: how review runs, how a lesson ends and how long it runs, how a session opens, the teaching language, the colours, and in curriculum mode their hours and date. They change it whenever they like; **Save** rewrites `preferences.js` and tells you what changed. Built from [templates/preferences.html](./templates/preferences.html) — see [Preferences](#preferences).
...
- `preferences.js` — the values behind `preferences.html`, plain JS data like `reviews.js`. The console reads it to shape the Due panel; you read it every session. Schema, and what each setting makes you do: [formats/preferences.md](./formats/preferences.md). Beside it, `theme.css` is the picked colour set, copied from `assets/themes/` on Save; `style.css` imports it, and no `theme.css` means the default.
...
- A deadline sets **pace**: keep milestones-left against weeks-left in `NOTES.md`, and say out loud when the learner is behind. **Coverage is fixed; the date moves.** Pace never trims what gets taught — if the hours do not fit, the honest move is a later date, not a thinner milestone. At the diagnostic, write the budget in `NOTES.md` in one fixed shape, so the verdict cannot soften in the prose: **`Budget: H h/wk × W wk = T h. Typical for this exam: A–B h. Verdict: below range / within range.`** *Below* means T is under A — not "the tight end"; if the usual range is 100–200 h and the budget is 70, say below. Then keep a second line current: **`Lessons left: N open topics ≈ N lessons at ~L h each = X h → Y weeks at H h/wk; fits by <date> / needs <later date>.`** Recompute it after every pre-assessment, because that is when N becomes real — and when a pre-assessment leaves most of a milestone `open` and the line says *needs a later date*, propose the date **in that same session**, not at the end of the milestone: the learner should hear in week one that the date will not hold, not in week twelve. For an exam, the workspace deliberately holds no exam-format practice — so book it outside: a weekly slot in the pace, a practice-exam source in `sources.html`, and ask each session how the last run went; the exam date should follow those scores, not the calendar.
...
**What an assessment is.** Plain multiple choice and short answers, each block tagged with the topic's name from `curriculum.html`. **Every sub-item the syllabus names gets probed** — a blueprint line like *"IPv6 address types (global, unique local, link local, anycast, multicast, modified EUI-64)"* is six things, not one, and a certification asks about each. Make it arithmetic, not intent: **count the items in the blueprint line** — everything in its parentheses or its serial list, and the line itself when it names nothing (*"verify IP parameters for Client OS (Windows, Mac OS, Linux)"* is three) — and write at least that many questions for the topic, one per item, split between pre and post if that makes the page long; a question that bundles three items into one option probes one, not three. A heavy topic (subnetting) gets more than its count; a light one never gets fewer. Put the count beside the topic as you write (`<!-- 1.10: 3 items, 3 questions -->`) so the page can be checked against it. Keep the options of a question the same length and shape, and move the right one around — the template says why — and **measure it before you link the page**: `bin/check-quiz assessments/m1-pre.html` counts how often the right option is the longest (chance is one in three; the ceiling is 40%) and lists the questions to rewrite. Writing to the rule is not enough — a page written with care still came out at 79% — so the check gates the link: a page that prints `REWRITE` is not offered to the learner. A short answer left blank counts as missed whatever the learner presses; the page enforces it. It is written in the teaching language like everything else. It is **not** a simulation of the real exam — no exam formats, no timer, no drag-and-drop. Its job is to challenge what the learner holds about the milestone; practising the exam's own format is the learner's preparation, not the workspace's, and say so if they ask. The page scores itself (first pick counts) and ends with **Send to tutor**, which lands the whole thing in `submissions/` and announces it in your terminal ([Submissions](#submissions)). Opened off disk there is no button, only a result line — `m1-post 7/10 missed: vlan trunking, stp` — that the learner reports to you; ask for it if nothing arrives.

**The pre-assessment is a filter.** The opening diagnostic reads the learner in broad strokes; this reads them topic by topic, which no conversation could. Write it from the milestone's topics, tell the learner where it is in the console — the page will show a **new content** badge; that brings it in — and take the result at face value: a topic becomes `known` in `curriculum.html` — no lesson for it — only when **every one of its sub-items** held — which means it was asked about every one: a topic with fewer questions than items cannot become `known`, whatever the score. Two questions cannot clear an eight-item topic, and one bundled question cannot clear three. One sub-item missed and the topic stays `open`, with the lesson aimed at what missed. If it clears the whole milestone, set the milestone `known` and hand the choice back — the learner starts the next one from the menu; otherwise keep it `active` and teach the `open` topics. Frame it as it is: there is nothing here to pass or fail, only time saved.

**The post-assessment is an honest reading.** Once the last `open` topic is taught, write it over **all** the milestone's topics — the `known` ones too, because clearing a pre-assessment was a claim and this checks it. It can follow the last lesson the same day — but with **fresh questions**: never a lesson's quick check re-asked, inverted or with the numbers swapped, because that reads recall of the page, not what transferred. Come at each sub-item from a different angle than the lesson did. What did not hold is a fact about the material, not about the learner: say so plainly.
