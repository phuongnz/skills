...
- **Settle the mode.** Ask: *is there a syllabus or exam this has to cover?* If not, record `Mode: open` in `NOTES.md` and carry on. If there is, the workspace runs in [curriculum mode](./SKILL.md#two-modes-open-and-curriculum):
  ...
  - Get the date, if any, into the Goal's constraints — and do the arithmetic out loud: hours a week × weeks to the date, against what this exam usually takes. Below range, say so now, not at the end; the date is the variable, not the coverage ([pace](./SKILL.md#two-modes-open-and-curriculum)).
