...
**Per task** (a task = one user-facing unit of work — one ticket, one PR, one query):
...
| Context-heavy (RAG, long history, many tools) | 20k–100k | 1k–4k |
...
- **Memory / vector store** — managed vector DB monthly, or self-host. Add embedding cost: one-time index + per-query embed (small vs. generation, but include it for RAG designs).
...
Ask the user for: **tasks/period**, **avg context size** (or take it from the design), **model tier**, **human-review volume & rate**, and the **pricing regime**. It is fine to ask.
...
> ⚠ *Assumed:* 1,000 tasks/day (~30k/month) · ~8k input / 1.5k output tokens per call · rung-1 baseline, retry_factor 1.2 · prices = the live ones fetched today.
